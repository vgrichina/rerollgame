...
let publishedGame = null; // {gameCode, metadata, description} when playing a published post
...
async function init() {
  try {
    ...
    if (data.type === 'game' && data.gameCode) {
      // Published game post — go straight to playing
      publishedGame = {
    ...
    // /api/init failed — fall through to creator mode
  }

  // Creator mode (starter post or no game)
...
function getCurrentVersion() {
  if (publishedGame) return publishedGame;
...
async function renderPlaying() {
  ...
  const isPublished = !!publishedGame;
  ...
  const gameOverButtons = isPublished
    ? `<button id="preview-go-replay" style="background:var(--primary); color:var(--text-1); border:none; border-radius:16px; padding:10px 28px; font:bold 14px var(--font-body); cursor:pointer;">PLAY AGAIN</button>
       <button id="preview-go-create" style="background:transparent; color:var(--text-2); border:1px solid var(--border-light); border-radius:16px; padding:6px 16px; font-size:12px; cursor:pointer;">MAKE YOUR OWN</button>`
    : `<button id="preview-go-replay" style="background:var(--primary); color:var(--text-1); border:none; border-radius:16px; padding:10px 28px; font:bold 14px var(--font-body); cursor:pointer;">PLAY AGAIN</button>
  ...
  if (isPublished) {
    document.getElementById('preview-go-create')?.addEventListener('click', () => {
      stopPreviewGame();
      publishedGame = null;
      rollAll();
      loadDrafts();
